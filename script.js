...
function isEmpty(v){ return !v||String(v).indexOf('—')>=0||String(v).indexOf('À')>=0; }
...
function renderProfil(e){
  ...
  if(tagsEl){
    var tags=[{k:'CIN',v:e.cin},{k:'CNE',v:e.cne},
              {k:'Né(e) le',v:e.date_naissance},{k:'Ville',v:e.ville},{k:'Tél',v:e.telephone}];
    tagsEl.innerHTML=tags.map(function(t){
      return '<span class="id-tag"><strong>'+t.k+' :</strong> '+(isEmpty(t.v)?'—':t.v)+'</span>';
    }).join('');
  }
